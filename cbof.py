...
class BoF_Pooling(Layer):
    ...
    def build(self, input_shape):
        ...
        self.V = self.add_weight(name='codebook', shape=(1, 1, input_shape[3], self.N_k), initializer='uniform',
                                 trainable=True)
#        self.V = self.add_weight(name='codebook', shape=tf.convert_to_tensor((1, 1, input_shape[3], self.N_k), dtype=tf.float32)
...
def initialize_bof_layers(model, data, n_samples=100, n_feature_samples=5000, batch_size=32, k_means_max_iters=300,
    ...
    for i in range(len(model.layers)):
        if isinstance(model.layers[i], BoF_Pooling):
            ...
            get_features = K.function([model.input], [model.layers[i].input])
...
#            get_features = K.function([model.input] + [K.learning_phase()], [model.layers[i - 1].get_output_at(-1)])
